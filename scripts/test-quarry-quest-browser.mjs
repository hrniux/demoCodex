...
const TEST_URL = buildTestUrl({
  envName: 'QUARRY_QUEST_TEST_URL',
  pathname: '/quarry-quest.html',
  query: '?autotest=1&seed=4242',
});
const SCREENSHOT_DIR = path.resolve(process.cwd(), 'output/quarry-quest-browser');
const CAPTURE_SCREENSHOTS = process.env.QUARRY_QUEST_CAPTURE === '1';

async function runBasicRoute(page) {
  await openStartedPage(page, { url: TEST_URL, startSelector: '#game-start' });

  await page.evaluate((setup) => {
    const game = window.quarryQuestGame;
...
async function runExtractionScenario(page) {
  ...
  assert.equal(afterExtraction.score, 440);
