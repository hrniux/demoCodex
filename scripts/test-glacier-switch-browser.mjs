...
import glacierSwitchModule from '../src/js/glacier-switch.js';
import {
  buildTestUrl,
...
const { CONFIG } = glacierSwitchModule;
...
async function applyScenario(page, setup) {
  await page.evaluate((scenario) => {
    ...
    const game = window.glacierSwitchGame;
    if (!game) {
      throw new Error('Game global not found: glacierSwitchGame');
    }
...
async function assertInternal(page, expected) {
  ...
  const payload = await page.evaluate(() => ({
    freezeTurns: window.glacierSwitchGame.state.freezeTurns,
    hazards: window.glacierSwitchGame.state.hazards.length,
  }));
...
const testUrl = buildTestUrl({
  envName: 'GLACIER_SWITCH_TEST_URL',
  pathname: '/glacier-switch.html',
  query: '?autotest=1&seed=4242',
});
const captureEnabled = process.env.GLACIER_SWITCH_CAPTURE === '1';
const screenshotDir = path.resolve(process.cwd(), 'output/glacier-switch-browser');
